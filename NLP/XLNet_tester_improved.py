...
# 加载嵌入数据，并移动到GPU
embeddings_load_path = "C:/Users/Zeyi/PycharmProjects/COMP8880/improved_XLNet_embeddings_data_50_product_meta_more_infos.npy"
embeddings = np.load(embeddings_load_path)
embeddings = torch.tensor(embeddings, device=device)

def get_xlnet_embedding(text, model, tokenizer, device):
    inputs = tokenizer(text, return_tensors='pt', max_length=512, truncation=True, padding='max_length')
    inputs = {key: value.to(device) for key, value in inputs.items()}  # 将输入移动到GPU
    outputs = model(**inputs)
    return outputs.last_hidden_state[:, -1, :]  # 输出在GPU上
...
def combine_weighted_embedding(title, description, feature, category, brand, model, tokenizer, device,
                               ...
                               brand_weight=1.0):
    title_embedding = get_xlnet_embedding(title, model, tokenizer, device)
    description_embedding = get_xlnet_embedding(description, model, tokenizer, device)
    feature_embedding = get_xlnet_embedding(feature, model, tokenizer, device)
    category_embedding = get_xlnet_embedding(category, model, tokenizer, device)
    brand_embedding = get_xlnet_embedding(brand, model, tokenizer, device)
...
output_file_path = "C:/Users/Zeyi/PycharmProjects/COMP8880/dataset/XLNet_improved_similar_products_data_50_product_meta_more_infos.txt"
